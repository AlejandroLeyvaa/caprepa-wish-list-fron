...
export class UserComponent implements OnInit {
  ...
  data: any;
  getUserImageUrl: any = '';
  URL: string = 'http://192.168.1.116:3000/';
  ...
  async getCurrentUserData() {
    ...
    if (userId && token?.length) {
      const URL = `http://192.168.1.116:3000/api/auth/users/${userId}`;
  ...
  async sendUserImage(formData: any) {
    const response = await fetch(`http://192.168.1.116:3000/api/auth/users/user-update-image`, {
      method: 'PUT',
      body: formData,
    });
  ...
  async sendData(dataForm: any) {
    const response = await fetch(`http://192.168.1.116:3000/api/auth/users/user-update`, {
      method: 'PUT',
      headers: {
