...
export class UserComponent implements OnInit {
  ...
  async getCurrentUserData() {
    ...
    if (userId && token?.length) {
      const URL = `https://3cf1-187-149-136-216.ngrok.io/api/auth/users/${userId}`;
  ...
  async sendUserImage(formData: any) {
    const response = await fetch(`https://3cf1-187-149-136-216.ngrok.io/api/auth/users/user-update-image`, {
      method: 'PUT',
      body: formData,
    });
  ...
  async sendData(dataForm: any) {
    const response = await fetch(`https://3cf1-187-149-136-216.ngrok.io/api/auth/users/user-update`, {
      method: 'PUT',
      headers: {
