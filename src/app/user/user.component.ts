...
export class UserComponent implements OnInit {
  ...
  data: any;
  getUserImageUrl: any = '';
  URL: string = 'https://9a4a-201-164-62-254.ngrok.io/';
  ...
  async getDepartments() {
    const request = await fetch(`https://9a4a-201-164-62-254.ngrok.io/api/auth/departments`);
  ...
  async getCurrentUserData() {
    ...
    if (userId && token?.length) {
      const URL = `https://9a4a-201-164-62-254.ngrok.io/api/auth/users/${userId}`;
  ...
  async sendUserImage(formData: any) {
    const response = await fetch(`https://9a4a-201-164-62-254.ngrok.io/api/auth/users/user-update-image`, {
      method: 'PUT',
      body: formData,
    });
  ...
  async sendData(dataForm: any) {
    const response = await fetch(`https://9a4a-201-164-62-254.ngrok.io/api/auth/users/user-update`, {
      method: 'PUT',
      headers: {
